...
| `/writing` | PostList | 전체 글 (섹션·태그 필터) |
...
- [ ] **Phase 1. 뼈대**: Quartz 제거, Vite + Solid + MDX 설정, SSR entry, MDX 글 하나를 prerender해서 정적 HTML로 출력
...
- [ ] 폴더 매핑: `수학정리 → notes/math`, `reading → notes/reading`, `project/memo → research`, `project/회고 → projects`가 맞는지
- [ ] 42 SEOUL, MIT, MCQA 회고를 프로젝트 페이지로 볼지, 글로 둘지
- [ ] 글 목록 경로 이름: `/writing` vs `/blog`
